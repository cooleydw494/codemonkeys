...
from config.defs import STOR_MONK_PATH, nl2, nl
...
def run_help(monk_args: argparse.Namespace = None):
    ...
    # Overview
    print_t("Welcome to CodeMonkeys, an AI-ready automations core! The Monk CLI aims to prioritize simplicity, "
            "power, and extensibility. It provides hard-coded core utility and transforms the commands directory "
            "into readily usable CLI commands. Additionally, its flags make it an essential tool for both direct use "
            f"and extension of the core.{nl}", 'white')

    # Recursive Name-Matching Logic
    print_t("`monk` employs recursive name-matching logic to locate core/usr entities. This requires unique "
