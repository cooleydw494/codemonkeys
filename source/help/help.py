...
from source.utils.monk.theme.theme_functions import print_banner, print_table, print_t, apply_t
...
def run_default_help(monk_args: argparse.Namespace = None):
    ...
    # Overview
    print_t("Welcome to CodeMonkeys, an AI-ready automations source! The Monk CLI aims to prioritize simplicity, "
            "power, and extensibility. It provides hard-coded source utility and transforms the commands directory "
            "into readily usable CLI commands. Additionally, its flags make it an essential tool for both direct use "
            f"and extension of the codemonkeys-source.{nl}", 'white')

    # Recursive Name-Matching Logic
    print_t("`monk` employs recursive name-matching logic to locate source/usr entities. This requires unique "
    ...
    monk_general_file = os.path.join(STOR_MONK_PATH, 'monk-commands-general.json')
    ...
    monk_type_file = os.path.join(STOR_MONK_PATH, 'monk-commands-type.json')
    ...
    monk_action_file = os.path.join(STOR_MONK_PATH, 'monk-commands-action.json')
