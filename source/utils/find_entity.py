...
from defs import nl, COMMANDS_PATH, AUTOMATIONS_PATH, BARRELS_PATH, MODULES_PATH, CORE_PATH, nl2
from source.utils.framework_utils import levenshtein_distance
from source.utils.monk.theme.theme_functions import print_t, input_t
...
def find_entity(entity_name: str, entity_type: str, path_override: str = None) -> str:
    ...
    if matches:
        ...
        # If entity_type is 'module' and no matches found, search in 'source' path
        if entity_type == 'module':
            print_t(f"No module matches found for '{entity_name}'. Searching in source files...", 'warning')
            return find_entity(entity_name, 'source')
