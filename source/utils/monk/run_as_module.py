...
def run_as_module(module_path, function_name='main', monk_args=None):
    ...
    if hasattr(module, function_name):
        ...
        # Default Behavior:
        # Assuming the function takes args, namely the source-intended monk_args
        # (ex: add-monkey.py@main)
        if args_count == 1:
            return function(monk_args)

        # If the function takes no args, just run it. This should work IMO, but source commands will always define
